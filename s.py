...
def ui():
    ...
    # Install pip dependencies for legacy ComfyUI-Manager custom node
    print("Installing pip dependencies for legacy ComfyUI-Manager custom node...")
    legacy_manager_req_path = os.path.join(CUSTOM_NODES_DIR, "comfyui-manager", "requirements.txt")
    if os.path.exists(legacy_manager_req_path):
        try:
            result = subprocess.run(
                f"pip install -r {legacy_manager_req_path}",
                shell=True, check=True, capture_output=True, text=True
            )
            print("Legacy ComfyUI-Manager dependencies installed:", result.stdout)
        except subprocess.CalledProcessError as e:
            print(f"Error installing legacy Manager dependencies: {e.stderr}")
    else:
        print(f"Warning: {legacy_manager_req_path} not found, skipping legacy Manager dependencies installation")
